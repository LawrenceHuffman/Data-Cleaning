...
age_median = int(pd.to_numeric(df_no_missing["age"], errors="coerce").median())
df_no_missing["age"] = pd.to_numeric(df_no_missing["age"], errors="coerce").fillna(age_median)
...
df_typed["join_date"] = pd.to_datetime(df_typed["join_date"], errors="coerce", format="mixed")
df_typed["last_purchase"] = pd.to_datetime(df_typed["last_purchase"], errors="coerce", format="mixed")
...
df_text_cleaned["phone"] = df_text_cleaned["phone"].fillna("(000) 000-0000")
...
df_no_duplicates["purchase_frequency_category"] = np.select(
    [
        ...
        df_no_duplicates["total_purchases"].between(5, 9, inclusive="both"),
...
print("- Categories: Standardized to Menswear, Womenswear, Footwear, and Accessories")
...
print(f"- Satisfaction/Spend Correlation: {satisfaction_spend_corr:.2f}")
